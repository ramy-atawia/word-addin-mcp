...
async def _llm_intent_detection(state: AgentState) -> tuple[str, List[Dict]]:
    ...
    prompt_parts = [
        ...
        "- \"prior art search\" + any topic → TOOL_WORKFLOW (prior art search workflow)",
        "- \"patent search X\" → TOOL_WORKFLOW (prior art search workflow)",
...
Create a response that the user will find immediately valuable and actionable based on the real research performed."""
    ...
    response = llm_client.generate_text(
        prompt=prompt,
        max_tokens=16384  # Maximum token limit for comprehensive analysis of all tool outputs
    )
