        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    logger.debug(f"🔍 LLM DEBUG - Model: {self.azure_openai_deployment}")
                    logger.debug(f"🔍 LLM DEBUG - Requested tokens: {max_tokens}")
                    logger.debug(f"🔍 LLM DEBUG - API params: {api_params}")
