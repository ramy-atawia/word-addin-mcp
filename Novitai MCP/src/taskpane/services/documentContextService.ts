...
import { officeIntegrationService, DocumentMetadata, DocumentStats } from './officeIntegrationService';
...
export class DocumentContextService {
  ...
  private detectWebContentNeeds(content: string, _selectedText: string): boolean {
    ...
    const researchKeywords = ['research', 'study', 'investigation', 'analysis', 'survey', 'findings', 'results', 'conclusion'];
  ...
  private detectDocumentAnalysisNeeds(content: string, _selectedText: string): boolean {
    ...
    const hasStructuredContent = content.includes('1.') || content.includes('•') || content.includes('-');
  ...
  private detectResearchContent(content: string): boolean {
    const researchKeywords = ['research', 'study', 'investigation', 'analysis', 'survey', 'findings', 'results', 'conclusion', 'methodology', 'hypothesis'];
  ...
  private detectTechnicalContent(content: string): boolean {
    const technicalKeywords = ['algorithm', 'function', 'method', 'class', 'interface', 'api', 'protocol', 'framework', 'library', 'database', 'server', 'client'];
  ...
  private getSurroundingContext(content: string, selectedText: string): string {
    try {
      ...
      const afterText = content.substring(index + selectedText.length, index + selectedText.length + 100);
