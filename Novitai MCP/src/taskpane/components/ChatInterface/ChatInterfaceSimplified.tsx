  ...
  const generateMessageId = useCallback(() => {
    ...
    const handleDocumentModification = async (userRequest: string) => {
      ...
      addMessage({
        ...
        console.log('Document modification tool response:', result);
        
        if (result.success && result.result) {
          console.log('Backend result:', result.result);
          console.log('Modifications to apply:', result.result.modifications);
          
          // Validate modifications before applying
          if (!result.result.modifications || !Array.isArray(result.result.modifications)) {
            throw new Error(`Invalid modifications format: ${JSON.stringify(result.result.modifications)}`);
          }
          
          if (result.result.modifications.length === 0) {
            throw new Error('No modifications generated by the backend');
          }
